...
function run({reduced=false,hanging=false,skip=false}={}) {
 ...
 const logo={decode:()=>Promise.reject(new Error('image unavailable')),getBoundingClientRect:()=>({left:24,top:28,width:164,height:11})};
 ...
 vm.runInNewContext(source,{window,document,matchMedia:()=>({matches:reduced}),setTimeout:fn=>{timers.push(fn);return timers.length;},clearTimeout(){},Event:class{},Promise});
...
test('logo flies to the header and unlocks content even if an image fails',async()=>{
 ...
 assert.equal(a.animations.length,1);assert.match(a.animations[0].frames[1].transform,/-352px/);
});
