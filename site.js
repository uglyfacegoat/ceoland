...
(() => {
  ...
  const selected = params.get('colour') === 'white' || screen === 'wallet-white' ? 'white' : 'black';
  ...
  const review = (items=activeCart) => {
    ...
    return `<div class="stack summary"><h3>ТВОЙ ОБЪЕКТ</h3><img class="summary-photo" src="${photo(list[0].colour)}" alt="CEOWALLET — ${colourName(list[0].colour)}" width="624" height="510">${list.map(x=>row(`CEOWALLET × ${x.quantity}`,money(x.quantity*PRICE))+row('Цвет',colourName(x.colour))).join('')}${row('Доставка','По адресу')}${rule}<p class="total">${money(total(list))}</p><p class="small muted">Стоимость объекта без доставки</p></div>`;
  };
  function product() {
    ...
    return intro('CEOWALLET.','OBJECT 01 / LIMITED EDITION')+`<div class="product-detail"><p class="detail-price">${money(PRICE)}</p><div class="product-gallery"><img class="gallery-main" id="gallery-main" src="${images[0]}" alt="CEOWALLET — ${colourName(c)}, вид спереди" width="624" height="510"><div class="gallery-thumbs" aria-label="Ракурсы">${images.map((src,i)=>`<button type="button" data-gallery="${src}" aria-label="Ракурс ${i+1}" aria-pressed="${i===0}"><img src="${src}" alt="" width="138" height="130"></button>`).join('')}</div><p class="gallery-caption">CEOMENTALITY / CEOWALLET　 •　 РАКУРСЫ</p></div><div class="stack product-information"><p>Натуральная кожа. Два цвета. Предмет из линейки CEOMENTALITY.</p><p>Покупка CEOWALLET не гарантирует вступление в клуб. Заявка на членство рассматривается отдельно.</p>${colours(c)}${row('КОЛЛЕКЦИЯ','Access')}${rule}<p class="small">Для покупки нужен действующий код доступа.</p><div class="action-stack">${link('КУПИТЬ ПО КОДУ','access',false,{colour:c})}${link('НЕТ КОДА? ЗАПРОСИТЬ','purchase-application',true,{colour:c})}</div><p class="small muted">Условия доставки и оплаты — при оформлении заказа.</p></div></div>`;
  }
  function catalog() {
    return intro('OBJECTS.','ЛИНЕЙКА ОБЪЕКТОВ CEOMENTALITY.<br>ПРОДУКЦИЯ ≠ ЧЛЕНСТВО.')+`<div class="catalog-list">${[
      ['wallet','CEOWALLET','НАТУРАЛЬНАЯ КОЖА',photo('black')],
    ...
    ].map(([r,name,material,src],i)=>`<article class="catalog-item"><div class="catalog-copy"><p class="eyeline accent">OBJ / 0${i+1}${i?' — СКОРО':''}</p><h2><a href="${route(r)}">${name}</a></h2><p class="small muted">${material}</p>${i?'<p class="accent" style="margin-top:32px">В РАЗРАБОТКЕ</p>':`<p class="detail-price">${money(PRICE)}</p>${colours('black')}<div class="catalog-arrows type-object" data-type="arrows" aria-hidden="true"></div>`}</div><a class="catalog-photo" href="${route(r)}" aria-label="Открыть ${name}"><img src="${src}" alt="${i?'Будущий объект под белой тканью':name}" width="664" height="543"></a></article>`).join('')}</div>`;
  }
  function teaser() {
  ...
  function formPage(kind) {
    ...
    const side=membership?`<div class="stack"><h2>РАССКАЖИ<br>О СЕБЕ.</h2><p>Мы внимательно изучаем каждую заявку. Оставь контакты, свой проект и цели — команда вернётся с ответом.</p>${slashes}</div>`:purchase?`<aside class="stack"><h3>ТВОЙ ОБЪЕКТ</h3><img class="summary-photo" src="${photo(selected)}" width="624" height="510" alt="CEOWALLET"><h3>CEOWALLET</h3><p>Это запрос кода для покупки CEOWALLET.</p><p class="muted">Анкета в клуб откроется отдельно — по QR внутри полученного картхолдера.</p></aside>`:`<aside class="stack"><h2>ПО ДЕЛУ.<br>НАПРЯМУЮ.</h2><p>Если вопрос связан с покупкой, укажи номер заказа. Для вступления в клуб есть отдельная заявка.</p>${link('КАК ВСТУПИТЬ','join',true)}</aside>`;
  ...
  function join() {return intro('КАК<br>ВСТУПИТЬ.','Покупка не открывает анкету автоматически. Она доступна по QR внутри полученного картхолдера.')+`<ol class="join-steps">${[['ПОЛУЧИ ОБЪЕКТ','Оформи покупку по коду и дождись доставки.'],['ОТСКАНИРУЙ QR','QR — на карточке внутри картхолдера.'],['ЗАПОЛНИ АНКЕТУ','Отправь анкету. Решение — после ручного отбора.']].map(([t,p],i)=>`<li><span class="join-number">0${i+1}</span><div><h3>${t}</h3><p>${p}</p></div></li>`).join('')}</ol>${slashes}`;}
  const faqItems=[
