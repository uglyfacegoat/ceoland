...
(() => {
  ...
  function join() {return intro('КАК<br>ВСТУПИТЬ.','Покупка не открывает анкету автоматически. Она доступна по QR внутри полученного картхолдера.')+`<ol class="join-steps">${[['ЗАЯВКА НА ПОКУПКУ', 'Оставь контакты, чтобы получить код доступа к покупке.'], ['ПОКУПКА', 'Введи код, выбери картхолдер и дождись доставки.'], ['ЗАЯВКА В КЛУБ', 'Открой анкету по QR внутри картхолдера и расскажи о себе.'], ['ОТБОР', 'Команда вручную рассматривает каждую заявку.'], ['ДА — ДА. НЕТ — НЕТ.', 'Сообщим решение. Если да — пришлём приглашение в клуб.']].map(([t,p],i)=>`<li><span class="join-number">0${i+1}</span><div><h3>${t}</h3><p>${p}</p></div></li>`).join('')}</ol>${slashes}`;}
  const faqItems=[
