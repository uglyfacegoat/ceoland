...
from build_models import (
    ...
    area_light, boolean, export_model, finalize_model, finish_normals,
    image_from_array, material, planar_uv, point_at, prism, round_outline, text_mesh, FONT_BOLD,
)
from leather_details import sewn_thread_mesh, pocket_transition_stitch, resample_pocket_surface, tension_relief, stitch_boundaries, STITCH_SPACING_MM

BASELINE = ROOT / "revisions" / "v1"
BODY_BULGE_MM = 0.20
MIDDLE_THICKNESS_MM = .70
FRONT_THICKNESS_MM = .65
BOTTOM_RADIUS_MM = 7.0
TOP_RADIUS_MM = .20
POCKET_TOP_RADIUS_MM = 2.2
BULGE_WEIGHTS = {
    "Leather backing": 0.22, "Backing painted edge": 0.22,
    "Back saddle stitching": 0.22, "Backing exposed top stitching": 0.22,
    "Inserted card": 0.72,
    "Middle leather pocket | unstitched upper edge": 0.86,
    "Middle side stitching left": 0.86, "Middle side stitching right": 0.86,
    **{f"Pocket transition stitch | {level} {side}": 0 for level in ("front", "middle") for side in ("left", "right")},
}
...
def load_existing(name):
    ...
    for obj in objects:
        ...
        obj.parent = None
        obj.data.update()
    root["revision"] = "v4 cardholder / v2 key | retained original meshes"
    return root, objects
...
def reference_grain(filename, crop, randomize_phase=False, illumination_filter=400):
    ...
    spectrum = np.fft.fft2(periodic) * (1-np.exp(-(fx*fx + fy*fy) * illumination_filter))
    if randomize_phase:
...
def refined_leather():
    natural = reference_grain("cardholder-material.png", (240, 730, 200, 200), illumination_filter=2500)
    ...
    grain = natural
    roughness = np.clip(0.64 - grain * 0.07, 0.54, 0.75)
    leather = material("Ivory | grained calf leather", (0.60, 0.58, 0.55), roughness=0.76)
    ...
    pbr_texture(leather, normal_pixels(grain, 1.8), "Normal", "leather-normal.png", 0.55)
    ...
    ink_shader.inputs["Base Color"].default_value = (0.0035, 0.018, 0.065, 1)
    ink_shader.inputs["Sheen Weight"].default_value = 0
    ink_shader.inputs["Specular IOR Level"].default_value = 0.18
    ink.diffuse_color = (0.0035, 0.018, 0.065, 1)
    # Keep print pigment independent of the leather's subtle colour variation.
    linear = np.clip(np.array([0.60, 0.58, 0.55]) * (1 + grain[:, :, None] * 0.10), 0, 1)
    encoded = np.where(linear <= 0.0031308, linear * 12.92, 1.055 * linear**(1/2.4) - 0.055)
    pixels = np.concatenate((encoded, np.ones((*grain.shape, 1))), axis=-1)
    image_from_array("leather-basecolor.png", pixels, TEXTURES / "leather-basecolor.png")
    color_image = bpy.data.images.load(str(TEXTURES / "leather-basecolor.png"), check_existing=False)
    color_image.colorspace_settings.name = "sRGB"
    color_image.pack()
    color_node = leather.node_tree.nodes.new("ShaderNodeTexImage")
    color_node.image = color_image
    leather.node_tree.links.new(color_node.outputs["Color"], shader.inputs["Base Color"])
    blue = material("Insert | coated midnight blue card", (0.0015, 0.009, 0.062), roughness=0.43)
    blue_shader = blue.node_tree.nodes.get("Principled BSDF")
    blue_shader.inputs["Specular IOR Level"].default_value = 0.28
    return leather, ink, blue


def binding_distance(x, y):
    if abs(x) > 45.5 and y < -30.5:
        return 7.0 - math.hypot(abs(x) - 45.5, y + 30.5)
    return min(52.5 - abs(x), y + 37.5)


def front_base_surface(x, y):
    cross = max(0, 1 - (x / 52.5) ** 2)
    lip = 0.08 * cross * math.exp(-((y - 14.5) / 2.0) ** 2)
    distance = binding_distance(x, y)
    rolled_edge = 0.08 * math.exp(-((distance - 0.85) / 0.8)**2)
    seam_compression = 0.072 * math.exp(-((distance - 2.3) / 0.8)**2)
    return 1.164 + lip + rolled_edge - seam_compression


def edge_compression(x, y):
    # The sewn border closes against the next skin; the cavity remains open inside.
    fraction = min(1, max(0, (binding_distance(x, y) - 2.7) / 2.3))
    return 1 - fraction * fraction * (3 - 2 * fraction)


def middle_lip(x, y):
    return .07 * max(0, 1 - (x / 52.5)**2) * math.exp(-((y - 25) / 4)**2)


def middle_surface(x, y):
    pull = (.472 + .64 * body_bulge(x, y) + middle_lip(x, y)) * edge_compression(x, y)
    compression = .028 * math.exp(-((abs(x) - 49.7) / .8)**2) * (.5 + .5 * math.cos((y - 17) * math.tau / 2.4))
    return .502 + middle_lip(x, y) - pull - compression


def front_surface(x, y):
    front = front_base_surface(x, y)
    middle_front = middle_surface(x, y) + .86 * body_bulge(x, y)
    pull = (front - FRONT_THICKNESS_MM + body_bulge(x, y) - middle_front + .02) * edge_compression(x, y)
    return front - pull + front_tension(x, y) + leather_lines(x, y)


def leather_lines(x, y):
    relief = 0
    for center_x, center_y, length, depth, phase in ((-12,-27,47,.05,.3),(9,-20,60,.045,1.8),(-18,-7,34,.04,2.4),(10,6,54,.035,.8),(4,-31,38,.035,3.5)):
        along = (x-center_x)/(length*.5)
        envelope = math.exp(-(along**6))
        line_y = center_y + .026*(x-center_x) + .30*math.sin(x*.10+phase)
        across = y-line_y
        relief -= depth*envelope*math.exp(-(across/.52)**2)
        relief += depth*.30*envelope*math.exp(-((across-.68)/.56)**2)
    return relief


@lru_cache(maxsize=1)
def front_needle_centers():
    outline = round_outline(100.4, 70.4, 4.7, steps=20, top_radius=.15)
    path = [(-50.2, 15.6)] + [(x, y) for x, y in outline if y < 15.6] + [(50.2, 15.6)]
    lengths = np.concatenate(([0], np.cumsum([math.dist(p, q) for p, q in zip(path, path[1:])])))
    stations = stitch_boundaries(lengths[-1], round(lengths[-1] / STITCH_SPACING_MM))
    return tuple(zip(np.interp(stations, lengths, [p[0] for p in path]), np.interp(stations, lengths, [p[1] for p in path])))


def front_tension(x, y):
    return tension_relief(x, y, front_needle_centers())


def body_bulge(x, y):
    distance = min(binding_distance(x, y), 37.5 - y)
    return BODY_BULGE_MM * (1 - math.exp(-(max(0, distance) / 6.0)**1.4))


def seam_geometry(name, outline, surface, thread, spacing=STITCH_SPACING_MM, closed=False, back=False):
    ...
    def at(distance):
        ...
        first, second = Vector(points[segment]), Vector(points[segment + 1])
        tangent = (second - first).normalized()
        fraction = (distance - lengths[segment]) / (lengths[segment + 1] - lengths[segment])
        return first.lerp(second, fraction), tangent

    count = max(1, round(lengths[-1] / spacing))
    stitch = sewn_thread_mesh(name, at, lengths[-1] / count, count, surface, thread, back,
                              slant_bottom=name == 'Front saddle stitching | sides and bottom')
    return [stitch]
...
def cardholder_seams():
    outline = round_outline(100.4, 70.4, 4.7, steps=20, top_radius=.15)
    front = [(-50.2, 15.6)] + [(x, y) for x, y in outline if y < 15.6] + [(50.2, 15.6)]
    upper = [(50.2, 28.1)] + [(x, y) for x, y in outline if y >= 28.1] + [(-50.2, 28.1)]
    return [
        ("Front saddle stitching | sides and bottom", front, front_surface, "Front leather pocket", False, False),
        ("Backing exposed top stitching", upper, lambda x, y: -0.65, "Leather backing", False, False),
        ("Middle side stitching left", [(-50.2, 17.35), (-50.2, 25.9)], middle_surface, "Middle leather pocket | unstitched upper edge", False, False),
        ("Middle side stitching right", [(50.2, 17.35), (50.2, 25.9)], middle_surface, "Middle leather pocket | unstitched upper edge", False, False),
        ("Back saddle stitching", outline, lambda x, y: -2, "Leather backing", True, True),
    ]


def pocket_transition_stitches(thread):
    backing = lambda x, y: -.65 + .22 * body_bulge(x, y)
    middle = lambda x, y: middle_surface(x, y) + .86 * body_bulge(x, y)
    front = lambda x, y: front_surface(x, y) + body_bulge(x, y)
    return [pocket_transition_stitch(f"Pocket transition stitch | {level} {side}", x, lip, lower, upper, thread)
            for level, lip, lower, upper in (("front", 16, front, middle), ("middle", 26.75, middle, backing))
            for side, x in (("left", -50.2), ("right", 50.2))]
...
def refine_cardholder(objects, detail=True):
    ...
    thread = material("Ivory | linen saddle thread", (0.68, 0.64, 0.58), roughness=0.86)
    thread.node_tree.nodes["Principled BSDF"].inputs["Specular IOR Level"].default_value = 0.18
    obsolete = {"Card navy stripe", "Front saddle stitching", "Front stitch impression", "Back saddle stitching", "Visible upper stitching", "Pocket interior", "Pocket painted edge"}
    ...
    new_back = round_outline(105, 75, BOTTOM_RADIUS_MM, top_radius=TOP_RADIUS_MM)
    ...
    for vertex in card.data.vertices:
        ...
        vertex.co.y -= 4.0
        vertex.co.z -= 0.31
        vertex.co.z = -.65 + (vertex.co.z + .65) * .4
    densify(card, 3.0)
    assign(card, blue)
    pocket = by_name["Front leather pocket"]
    assign(pocket, leather)
    middle = bpy.data.objects.new("Middle leather pocket | unstitched upper edge", bpy.data.meshes.new("Middle pocket"))
    bpy.context.collection.objects.link(middle)
    assign(middle, leather)
    to_mm = Matrix.Scale(1000, 4) @ Matrix.Rotation(-math.pi/2, 4, 'X')
    for panel, top, thickness, surface in ((pocket,16,FRONT_THICKNESS_MM,front_surface),(middle,26.75,MIDDLE_THICKNESS_MM,middle_surface)):
        resample_pocket_surface(panel,105,-37.5,top,BOTTOM_RADIUS_MM,POCKET_TOP_RADIUS_MM,thickness,surface)
        panel.data.transform(to_mm)
    objects.append(middle)
    if detail:
        for name, outline, surface, panel_name, closed, back in cardholder_seams():
            objects += seam_geometry(name, outline, surface, thread, closed=closed, back=back)
        objects += pocket_transition_stitches(thread)
    for name, old_center, new_center, width_scale in (
        ...
        densify(logo, 2.0)
        mesh = bmesh.new()
        mesh.from_mesh(logo.data)
        bmesh.ops.delete(mesh, geom=[face for face in mesh.faces if face.normal.z < .5], context='FACES')
        mesh.to_mesh(logo.data)
        mesh.free()
        for vertex in logo.data.vertices:
            x, y, z = vertex.co
            vertex.co.z = front_surface(x, y) + 0.045
        assign(logo, ink)
    # All visible layers share one padded envelope over the full cardholder height.
    for obj in objects:
        for vertex in obj.data.vertices:
            vertex.co.z += body_bulge(vertex.co.x, vertex.co.y) * BULGE_WEIGHTS.get(obj.name, 1.0)
        obj.data.update()
...
def main():
    ...
    for name, refinement in (("cardholder", refine_cardholder), ("key", refine_key)):
        ...
        reports.append(export_model(objects, root, name, split_materials=True))
