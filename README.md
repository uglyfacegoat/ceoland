...
Последняя сохранённая сцена с подгонкой по фото заказчика: `revisions/photorealism/client-photo-fit.blend`. Это рабочая версия с проекцией фронтальной фотографии на геометрию, а не финальный материал для произвольного освещения. Оригиналы фотографий находятся в `references/product-photos/`; текущий сайт — в `website/`. Предыдущие версии и пробные изображения сохранены для продолжения работы.

Текущий этап — исследование и переделка основы предметного рендера. Рабочая инструкция, источники, результаты проб и оставшиеся отличия: [photorealism-playbook.md](references/photorealism-playbook.md). Проверенная сцена: `revisions/photorealism/foundation-study.blend`, сцена `CEOMENTALITY | Web studio`. Она содержит новую кожу и печать; финальное освещение ещё дорабатывается. Команды ниже относятся к исторической версии v4.
