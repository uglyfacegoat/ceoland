...
Текущая версия сайта — фотооснова с приоритетом телефонов. Главный экран и блок о продукте получили отдельные мобильные кадры; широкие композиции сохранены для компьютера. Галерея, меню, коллекция, корзина и оформление переработаны. Ключ пока показан статично, четыре шага доступны без остановки прокрутки; анимация отложена по запросу пользователя.

План и решения: [mobile-photo-direction.md](references/mobile-photo-direction.md). Макет перед реализацией: [mobile-photo-layout.html](references/mobile-photo-layout.html). Новые изображения — генерации по референсам, не реальная съёмка; оригинальная фотография товара сохранена в галерее и корзине. Промпты: [mobile-photo-prompts.md](references/mobile-photo-prompts.md).
...
Публичные изображения доставляются в WebP с вариантами размера; исходники и старые модели вынесены в `previews/website-archive/`. Для повторного экспорта нужен FFmpeg в PATH: из `website/` выполнить `node scripts/prepare-images.mjs`. Полная сборка сайта занимает около 1,55 МБ без учёта серверного сжатия. Оплата и сервер заказов не подключены.
