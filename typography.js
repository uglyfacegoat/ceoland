/* Live typography. Figma letterforms and an square letter grid, rendered as live text. */
(() => {
  ...
  for (let row = 0; row < 12; row++) {
    [...'CEOMENTALITY'].forEach((text, column) => {
      square.glyphs.push({text, x:10 + column * 26, y:row * 26,
        color:row === 5 ? '#2451b2' : '#171717'});
    });
  }
  ...
  window.addEventListener('pointermove', event => {
    ...
    pointer.active = event.pointerType !== 'touch' && !motionDisabled() && !mobile.matches;
    startInteraction();
  }, {passive: true});
  ...
  function startInteraction() {
    if (frame || document.hidden || motionDisabled() || mobile.matches) return;
  ...
  function animateFlow(time) {
    ...
    for (const [object, item] of state) {
      if (!item.flow || !item.visible || document.hidden || motionDisabled() || mobile.matches) continue;
  ...
  function syncPlayback() {
    for (const item of state.values()) {
      ...
      const playing = item.visible && !document.hidden && !motionDisabled() && !(item.flow && mobile.matches);
  ...
  function build(object) {
    ...
    const original = kind === 'square' ? square : designs[kind][variant];
    ...
    if (config.nodeId) object.dataset.figmaNode = config.nodeId;
    const animations = [];
    if (kind === 'field') {
