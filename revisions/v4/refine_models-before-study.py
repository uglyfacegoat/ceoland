...
def refined_leather():
    ...
    pbr_texture(leather, normal_pixels(grain, 1.8), "Normal", "leather-normal.png", 0.65)
    ...
    linear = np.clip(np.array([0.72, 0.70, 0.66]) * (1 + grain[:, :, None] * 0.12), 0, 1)
...
def binding_distance(x, y):
    if abs(x) > 48 and y < -33:
        return 4.5 - math.hypot(abs(x) - 48, y + 33)
    return min(52.5 - abs(x), y + 37.5)


def front_surface(x, y):
    cross = max(0, 1 - (x / 52.5) ** 2)
    lip = 0.32 * cross * math.exp(-((y - 14.5) / 2.0) ** 2)
    distance = binding_distance(x, y)
    rolled_edge = 0.34 * math.exp(-((distance - 0.85) / 0.8)**2)
    seam_compression = 0.20 * math.exp(-((distance - 2.3) / 0.55)**2)
    return 2.9 + lip + rolled_edge - seam_compression


def body_bulge(x, y):
    distance = min(binding_distance(x, -abs(y)), 37.5 - abs(y))
    return 4.2 * (1 - math.exp(-(max(0, distance) / 6.0)**1.4))


def densify_pocket(pocket):
    mesh = bmesh.new()
    mesh.from_mesh(pocket.data)
    bmesh.ops.triangulate(mesh, faces=list(mesh.faces))
    for _ in range(10):
        edges = []
        for edge in mesh.edges:
            close_to_binding = min(binding_distance(v.co.x, v.co.y) for v in edge.verts) < 4.0
            if edge.calc_length() > (0.95 if close_to_binding else 2.6):
                edges.append(edge)
        if not edges:
            break
        bmesh.ops.subdivide_edges(mesh, edges=edges, cuts=1, use_grid_fill=True)
    bmesh.ops.triangulate(mesh, faces=list(mesh.faces))
    mesh.to_mesh(pocket.data)
    mesh.free()


def seam_geometry(name, outline, surface, thread, panel, spacing=2.55, closed=False, back=False):
    ...
    pitch = lengths[-1] / count
    sign = -1 if back else 1
    for index in range(count):
        spline = curve.splines.new("POLY")
        spline.points.add(6)
        for control, fraction in zip(spline.points, (0, 0.10, 0.25, 0.5, 0.75, 0.9, 1)):
            point, tangent = at(index * pitch + 0.16 + fraction * (pitch - 0.32))
            perpendicular = Vector((-tangent.y, tangent.x))
            point += perpendicular * math.sin(fraction * 2 * math.pi) * 0.055
            lift = -0.20 if fraction in (0, 1) else 0.025 + math.sin(fraction * math.pi) * 0.02
            control.co = (point.x, point.y, surface(point.x, point.y) + sign * lift, 1)
    for index in range(count if closed else count + 1):
        point, tangent = at(index * pitch)
        across = Vector((-tangent.y, tangent.x))
        major = (tangent + across * 0.5).normalized()
        minor = Vector((-major.y, major.x))
        start = len(hole_vertices)
        sides = 12
        for depth, taper in ((-0.55, 0.62), (0.5, 1.22)):
            for angle in np.linspace(0, 2 * math.pi, sides, endpoint=False):
                position = point + major * (0.23 * taper * math.cos(angle)) + minor * (0.125 * taper * math.sin(angle))
                hole_vertices.append((position.x, position.y, surface(position.x, position.y) + sign * depth))
        hole_faces.append(tuple(start + i for i in reversed(range(sides))))
        hole_faces.append(tuple(start + sides + i for i in range(sides)))
        for i in range(sides):
            j = (i + 1) % sides
            hole_faces.append((start+i, start+j, start+sides+j, start+sides+i))
    if back:
        hole_faces = [tuple(reversed(face)) for face in hole_faces]
    stitch = bpy.data.objects.new(name, curve)
    bpy.context.collection.objects.link(stitch)
    curve.materials.append(thread)
    active(stitch)
    with bpy.context.temp_override(object=stitch, active_object=stitch, selected_objects=[stitch], selected_editable_objects=[stitch]):
        bpy.ops.object.convert(target="MESH")
    mesh = bpy.data.meshes.new(name + " needle punches")
    ...
    impression = bpy.data.materials.get("Leather | exposed needle recess")
    if impression is None:
        impression = material("Leather | exposed needle recess", (0.31, 0.285, 0.245), roughness=0.95)
    mesh.materials.append(impression)
    boolean(panel, holes)
    # Recess walls must not pull the exterior leather normals into the puncture.
    mesh_edit = bmesh.new()
    mesh_edit.from_mesh(panel.data)
    for edge in mesh_edit.edges:
        if len({face.material_index for face in edge.link_faces}) > 1:
            edge.smooth = False
    mesh_edit.to_mesh(panel.data)
    mesh_edit.free()
    panel["needle_punctures"] = panel.get("needle_punctures", 0) + count + (0 if closed else 1)
    return [stitch]
...
def refine_cardholder(objects, detail=True):
    ...
    thread = material("Ivory | linen saddle thread", (0.58, 0.558, 0.518), roughness=0.9)
    ...
    binding_points = [(x, y, 0) for x, y in perimeter if y <= 25.95]
    binding_points.insert(0, (-51.9, 25.95, 0))
    binding_points.append((51.9, 25.95, 0))
    binding = curve_tube("Joined leather side and bottom binding", binding_points, 0.34, leather, resolution=3)
    for vertex in binding.data.vertices:
        vertex.co.z = vertex.co.z * 3.9 - 0.30
    ...
    if detail:
        ...
        objects += seam_geometry("Front saddle stitching | sides and bottom", front_u, front_surface, thread, pocket)
        ...
        objects += seam_geometry("Backing exposed top stitching", upper, lambda x, y: -0.65, thread, backing)
        for x in (-50.2, 50.2):
            objects += seam_geometry("Middle side stitching " + ("left" if x < 0 else "right"), [(x, 16.3), (x, 26.4)], lambda x, y: 1.65, thread, middle)
        objects += seam_geometry("Back saddle stitching", upper_outline, lambda x, y: -2.0, thread, backing, closed=True, back=True)
