...
В корне репозитория находится `vercel.json`. При импорте GitHub-репозитория оставьте **Root Directory: `./`** и задайте **Node.js 24.x** в настройках проекта. Конфигурация запускает `npm --prefix website ci`, затем `npm --prefix website run build`, публикует только `website/dist`. Модели, документация и исходники сайта не входят в публичный каталог.

`framework: null` выбирает статическую публикацию (Other): Vite по-прежнему выполняет сборку, после которой наш скрипт создаёт HTML страниц. `cleanUrls: true` сопоставляет `/product`, `/checkout`, `/request/sent` и другие адреса с соответствующими `.html`. `trailingSlash: false` приводит адреса к виду без завершающего слеша. Готовый `404.html` используется для неизвестных маршрутов. Общий rewrite на `index.html` добавлять не нужно.

Пока домен не выбран, оставьте `VITE_SITE_URL` пустым. Для индексируемого запуска добавьте настоящий HTTPS-origin в переменную `VITE_SITE_URL` **только окружения Production**, затем выполните Redeploy. В окружении Preview переменную не задавайте: тестовые сборки останутся закрытыми от индексации. Перед релизом отдельно выполните локальную проверку `npm run build:release` из `website` с настроенным доменом, как описано выше.

После первого деплоя проверьте прямое открытие и обновление `/product`, `/checkout`, `/request/sent`, собственные title этих страниц, HTTP 404 для неизвестного пути, а также robots.txt, sitemap.xml и canonical на выбранном домене. Конфигурация подготовлена локально; работа на инфраструктуре Vercel проверяется после создания деплоя.

Основания: [конфигурация Vercel](https://vercel.com/docs/project-configuration/vercel-json), [статическая страница 404](https://vercel.com/kb/guide/custom-404-page), [версии Node.js](https://vercel.com/docs/functions/runtimes/node-js/node-js-versions).
