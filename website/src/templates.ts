        ...
        ${collectionProducts.map((product) => `
          <article class="collection-card ${product.live ? "is-live" : "is-soon"}">
            <div class="collection-media">
              ${product.live ? productPhoto({ blueCard: false }) : `<img src="/images/editorial/${product.image}-${product.width}.webp" srcset="/images/editorial/${product.image}-${product.width}.webp ${product.width}w, /images/editorial/${product.image}-${product.largeWidth}.webp ${product.largeWidth}w" sizes="(min-width: 1800px) 360px, (min-width: 1024px) 24vw, 76vw" alt="${product.name}" loading="lazy" decoding="async" width="1254" height="1254" />`}
...
function accessStory() {
  ...
  return `
    <section class="access-story" id="how" aria-labelledby="access-story-title">
        ...
        <div class="access-visual"><img src="/images/editorial/key-still-960.webp" srcset="/images/editorial/key-still-480.webp 480w, /images/editorial/key-still-960.webp 960w, /images/editorial/key-still-1448.webp 1448w" sizes="(min-width: 1600px) 700px, (min-width: 1024px) 42vw, (min-width: 680px) 600px, 90vw" alt="Металлический ключ с гравировкой ACCESS" width="1448" height="1086" loading="lazy" decoding="async" /></div>
  ...
  return `
    <main class="home-page">
      ...
      <section class="hero" aria-labelledby="hero-title">
        ...
        <div class="hero-product">${productPhoto({ priority: true })}</div>
      ...
      <section class="product-intro" id="about" aria-labelledby="product-intro-title">
        ...
        <div class="product-intro-media">${productPhoto()}</div>
      </section>
      </div>
  ...
  return `
      <section class="product-detail" id="product" aria-labelledby="product-title">
        ...
        <div class="product-gallery">
          <div class="product-main-image" data-view="photo" tabindex="0" role="region" aria-label="Галерея картхолдера, используйте стрелки для переключения">${productPhoto({ priority: standalone })}</div>
        ...
        <div class="product-thumbnails">
          ${productImages.map((picture, index) => `<button class="${index === 0 ? "is-active" : ""}" type="button" data-view="${picture.view}" data-gallery-index="${index}" aria-current="${index === 0}" aria-label="${picture.label}"><div aria-hidden="true">${productPhoto()}</div><span>${picture.label}</span></button>`).join("")}
        </div>
  ...
  return `
    <main class="commerce-page checkout-page">
      ...
      <section class="checkout-shell">
        ...
        <div class="checkout-product">${productPhoto({ blueCard: false })}<span>CEOMENTALITY Access</span><strong>${formatRubles(total)}</strong><span>${quantity} шт.</span></div>
